...
contract UniftyGenesis is CloneFactory, Ownable, WhitelistAdminRole {
	...
	event PoolCreated(address indexed _user, address indexed _erc1155, uint256 _fee);
    ...
    function init( ERC1155Tradable _erc1155Address ) public {
	    require(!constructed && !isCloned, "UniftyGenesis must not be constructed yet or cloned.");
	    constructed = true;
	...
	function setErc1155(ERC1155Tradable _erc1155Address) external onlyWhitelistAdmin {
	    erc1155 = _erc1155Address;
	}
	...
	function newPool(
	    ...
	    emit PoolCreated(msg.sender, clone, nifBalance < poolFeeMinimumNif && !iHaveAnyWildcard() ? poolFee : 0);
	}
	...
	function getPool(address _address, uint256 index) external view returns(address){
	    require(!isCloned, "Not callable from clone");
