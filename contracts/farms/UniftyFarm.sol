...
import "contracts/lib/access/Pausable.sol";
import "contracts/lib/erc20/Wrap.sol";
import "contracts/lib/erc20/DetailedERC20.sol";
    ...
    WhitelistAdminRole
{
    ...
    event CardType(
        address indexed erc1155,
        uint256 indexed card,
        string indexed cardType
    );
    event CardShadowed(
        address indexed erc1155,
        uint256 indexed card,
        bool indexed shadowed
    );
    event Removed(
        address indexed erc1155,
        uint256 indexed card,
    ...
    function cardMintFee(address erc1155Address, uint256 id)
        external
        view
        returns (uint256)
    {
        return
            cards[erc1155Address][id].mintFee.add(
                cards[erc1155Address][id].controllerFee
            );
    }

    function cardReleaseTime(address erc1155Address, uint256 id)
        external
        view
        returns (uint256)
    {
    ...
    function cardPoints(address erc1155Address, uint256 id)
        external
        view
        returns (uint256)
    {
    ...
    function earned(address account) public view returns (uint256) {
        ...
        return points[account].add(getCurrPoints(account, pow));
    }

    function getCurrPoints(address account, uint256 pow)
        internal
        view
        returns (uint256)
    {
        ...
        whenNotPaused()
    {
        ...
        require(
            amount.add(balanceOf(msg.sender)) >= minStake &&
                amount.add(balanceOf(msg.sender)) > 0,
            "Too few deposit"
        );
        require(
    ...
    function redeem(address erc1155Address, uint256 id)
        external
        payable
        updateReward(msg.sender)
    {
        ...
        require(
            block.timestamp >= cards[erc1155Address][id].releaseTime,
            "Card not released"
        );
        require(
            points[msg.sender] >= cards[erc1155Address][id].points,
        ...
        if (enableFees) {
            ...
            pendingWithdrawals[controller] = pendingWithdrawals[controller].add(
                cards[erc1155Address][id].controllerFee
            );
            pendingWithdrawals[
                cards[erc1155Address][id].artist
            ] = pendingWithdrawals[cards[erc1155Address][id].artist].add(
                cards[erc1155Address][id].mintFee
            );
        }

        points[msg.sender] = points[msg.sender].sub(
            cards[erc1155Address][id].points
        );
        spentScore = spentScore.add(cards[erc1155Address][id].points);

        ERC1155Tradable(cards[erc1155Address][id].erc1155).safeTransferFrom(
            address(this),
            msg.sender,
            id,
            1,
        ...
        emit Redeemed(
            msg.sender,
            cards[erc1155Address][id].erc1155,
            id,
            cards[erc1155Address][id].points
        );
    }
    ...
    function removeNfts(
        ...
        ERC1155Tradable(_erc1155Address).safeTransferFrom(
            address(this),
            _recipient,
            _id,
            _amount,
    ...
    function createNft(
        ...
        uint256 tokenId =
            ERC1155Tradable(_erc1155Address).create(_supply, _supply, _uri, "");
    ...
    function supply(address _erc1155Address, uint256 _id)
        external
        view
        returns (uint256)
    {
    ...
    ) private onlyWhitelistAdmin {
        emit CardAdded(
            _erc1155Address,
            tokenId,
            _points,
            _mintFee.add(_controllerFee),
            _artist,
            _releaseTime
        );
    ...
    function init(
        ...
        require(
            ...
            "UniftyFarm must not be constructed yet or cloned."
        );
        require(
            _minStake >= 0 && _maxStake > 0 && _maxStake >= _minStake,
            "Problem with min and max stake setup"
    ...
    function newFarm(
        ...
        emit FarmCreated(
            msg.sender,
            clone,
            nifBalance < farmFeeMinimumNif && !iHaveAnyWildcard() ? farmFee : 0,
            _uri
        );
