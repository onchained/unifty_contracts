...
import "contracts/lib/math/SafeMath.sol";
...
contract UniftyFarmShopAddon is Ownable, CloneFactory, WhitelistAdminRole {
    ...
    address payable public feeAddress =
        address(0x2989018B83436C6bBa00144A8277fd859cdafA7D);
    ...
    bytes4 internal constant ERC1155_RECEIVED_ERR_VALUE = 0x0;

    event NewShop(
        address indexed _user,
        address indexed _farmAddress,
        address indexed _shopAddress
    );
    ...
    function obtain(
        ...
        require(
            runMode == 1 || runMode == 2,
            "UniftyFarmShopAddon#obtain: Farm not open for direct sales."
        );
        require(
            ERC1155Tradable(_erc1155Address).balanceOf(farm, _id) >= _amount,
            "UniftyFarmShopAddon#obtain: Desired amount exceeds stock."
        );

        bytes memory id = abi.encode(_id);
        require(
            prices[_erc1155Address][id] != 0 ||
                artistPrices[_erc1155Address][id] != 0,
            "UniftyFarmShopAddon#obtain: Price not set"
        );
        require(
            prices[_erc1155Address][id]
                .add(artistPrices[_erc1155Address][id])
                .mul(_amount) ==
                msg.value &&
                msg.value > 0,
            "UniftyFarmShopAddon#obtain: Invalid value"
        );

        (, , , , address _artist, , , , ) =
            UniftyFarm(farm).cards(_erc1155Address, _id);

        if (address(_artist) != address(0)) {
            address(address(uint160(_artist))).transfer(
                artistPrices[_erc1155Address][id].mul(_amount)
            );
            address(address(uint160(UniftyFarm(farm).controller()))).transfer(
                prices[_erc1155Address][id].mul(_amount)
            );
        } else {
            address(address(uint160(UniftyFarm(farm).controller()))).transfer(
                msg.value
        ...
        UniftyFarm(address(farm)).removeNfts(
            _erc1155Address,
            _id,
            _amount,
            msg.sender
    ...
    function getPrice(address _erc1155Address, uint256 _id)
        external
        view
        returns (uint256, uint256)
    {
        return (
            prices[_erc1155Address][abi.encode(_id)],
            artistPrices[_erc1155Address][abi.encode(_id)]
        );
    }
    ...
    function init() external {
        require(
            ...
            "UniftyFarmShopAddon must not be constructed yet or cloned."
        );
    ...
    function newAddon(address _farmAddress)
        external
        payable
        lock
        returns (address)
    {
        ...
        require(
            UniftyFarm(_farmAddress).owner() == msg.sender,
            "FarmShopAddon#newAddon: Not the farm owner"
        );
        ...
        if (!iHaveAnyWildcard()) {
            require(
                msg.value == addonFee,
                "FarmShopAddon#newAddon: Invalid addon fee"
            );
        }
    ...
    function setNifAddress(address _nifAddress) external onlyWhitelistAdmin {
        require(
            !isCloned,
            "FarmShopAddon#setNifAddress: Not callable from clone"
        );
        ...
        onlyWhitelistAdmin
    {
        require(
            !isCloned,
            "FarmShopAddon#setFeeAddress: Not callable from clone"
        );
    ...
    function setAddonFee(uint256 _addonFee) external onlyWhitelistAdmin {
        require(
            !isCloned,
            "FarmShopAddon#setAddonFee: Not callable from clone"
        );
        addonFee = _addonFee;
    }
    ...
    function setWildcard(uint256 wildcard) external onlyWhitelistAdmin {
        require(
            !isCloned,
            "FarmShopAddon#setWildcard: Not callable from clone"
        );
        ...
        onlyWhitelistAdmin
    {
        require(
            !isCloned,
            "FarmShopAddon#setWildcardErc1155Address: Not callable from clone"
        );
    ...
    function removeWildcard(uint256 wildcard) external onlyWhitelistAdmin {
        require(
            !isCloned,
            "FarmShopAddon#removeWildcard: Not callable from clone"
        );
